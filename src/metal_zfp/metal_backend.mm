...
static size_t
zfp_fallback_encode1d_int32(const int32* src, uint dim, int sx, uint maxbits, void* stream_words, size_t stream_capacity_bytes)
{
  zfp_field* field;
  zfp_stream* zfp;
  bitstream* bs;
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 4.0, zfp_type_int32, 1, zfp_false);
  field = zfp_field_1d((void*)src, zfp_type_int32, dim);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return 0;
...
static zfp_bool
zfp_fallback_decode1d_int32(const void* stream_words, size_t stream_bytes, int32* dst, uint dim, int sx, uint maxbits)
{
  zfp_field* field;
  zfp_stream* zfp;
  bitstream* bs;
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 4.0, zfp_type_int32, 1, zfp_false);
  field = zfp_field_1d(dst, zfp_type_int32, dim);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return zfp_false;
...
static size_t
zfp_fallback_encode2d_int32(const int32* src,
                            uint nx,
                            uint ny,
                            ptrdiff_t sx,
                            ptrdiff_t sy,
                            ...
                            size_t stream_capacity_bytes)
{
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 16.0, zfp_type_int32, 2, zfp_false);
  field = zfp_field_2d((void*)src, zfp_type_int32, nx, ny);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return 0;
...
static zfp_bool
zfp_fallback_decode2d_int32(const void* stream_words,
                            size_t stream_bytes,
                            int32* dst,
                            uint nx,
                            uint ny,
                            ...
                            uint maxbits)
{
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 16.0, zfp_type_int32, 2, zfp_false);
  field = zfp_field_2d(dst, zfp_type_int32, nx, ny);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return zfp_false;
...
static size_t
zfp_fallback_encode3d_int32(const int32* src,
                            uint nx,
                            uint ny,
                            uint nz,
                            ptrdiff_t sx,
                            ...
                            size_t stream_capacity_bytes)
{
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 64.0, zfp_type_int32, 3, zfp_false);
  field = zfp_field_3d((void*)src, zfp_type_int32, nx, ny, nz);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return 0;
...
static zfp_bool
zfp_fallback_decode3d_int32(const void* stream_words,
                            size_t stream_bytes,
                            int32* dst,
                            uint nx,
                            uint ny,
                            ...
                            uint maxbits)
{
  ...
  zfp_stream_set_rate(zfp, (double)maxbits / 64.0, zfp_type_int32, 3, zfp_false);
  field = zfp_field_3d(dst, zfp_type_int32, nx, ny, nz);
  if (!field) {
    zfp_stream_close(zfp);
    stream_close(bs);
    return zfp_false;
...
#else
  {
    ...
    if (field->type == zfp_type_int32 && dims[0] && dims[1] && !dims[2] &&
      ...
      if (!got) {
        ...
        size_t blocks = (px * py) / 16u;
        size_t stream_bytes = (blocks * (size_t)stream->maxbits + 7u) / 8u;
        if (zfp_fallback_decode2d_int32(stream_data(stream->stream),
                                        stream_bytes,
                                        (int32*)field->data,
                                        dims[0],
                                        dims[1],
